...
{
    return rio::fut::loop(std::move(ctx), [](ClientContext &ctx) {
        return rio::fut::make(std::move(ctx),
            ...
            .timeout_with(std::chrono::seconds(timeout_seconds),
                // Becuase this is transfer of ownership state is moved,
                // either accept by value or as r value: ClientContext&&
                [](ClientContext c) {
                    std::println("Client {} timed out. Sending goodbye...", c.addr);

                    return rio::fut::make(std::move(c), [](ClientContext &c) -> rio::fut::res<ClientContext> {
            ...
            .then([](ClientContext &&c) -> auto {
                // Becuase this is transfer of ownership state is moved, either accept
                // by value or as r value: ClientContext&&
                return rio::fut::make(std::move(c), [](ClientContext &c) -> rio::fut::res<ClientContext> {
                    using rio::fut::res;
...
auto main() -> int
{
    ...
    // Functions that have try return immediately on any error, like here
    // try_accept will retutn on would block. Same with try_read and try_write.
    auto acceptor = rio::Future{ std::move(listener), [](rio::Tcp_socket &l) -> rio::fut::res<ClientContext> {
        rio::address addr{};
    ...
    while (true) {
        ...
        for (int i = clients.size() - 1; i >= 0; --i) {
            // If we weren't using fut::loop, we would have to handle looping manually
            // like:
            //
