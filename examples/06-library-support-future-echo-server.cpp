...
auto make_client(rio::context &ctx, rio::Tcp_socket s, rio::address a)
{
    ...
    return rio::fut::loop(std::move(ptr), [&ctx](std::unique_ptr<Client> &ptr) {
        Client *c = ptr.get();
        return rio::fut::read(ctx, c->sock, c->buf)
            .then([c, &ctx](std::size_t n) {
            ...
            .then([](std::size_t n) {
                return rio::fut::make(n, [](std::size_t n) {
                    if (n == 0)
    ...
    }).or_else([](std::error_code code) {
        if (code!= std::errc::broken_pipe && code!= std::errc::connection_aborted)
            std::println(" [RIO]: task terminated with error: {}",  code.message());
...
{
    rio::context IO;
    auto server_sk = rio::Tcp_socket::open_and_listen(rio::address::any_ipv4(6969), rio::s_opt::async_server_v4)
                         .value();
    std::println(" [RIO]: Listening on 6969...");

    std::vector<ClientFuture> clients;

    auto server = rio::fut::loop(std::move(server_sk), [&](rio::Tcp_socket &listener) {
