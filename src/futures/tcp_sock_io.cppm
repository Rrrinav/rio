...
struct Accept_req
{
    ...
    static void on_complete(rio::internals::uring_request_header *ptr, int res)
    {
        ...
        if (res < 0) {
            ...
            p.resolve(
                Tcp_accept_result{ .client = rio::Tcp_socket::attach(res), .address = std::move(self->client_addr) });
        }
...
export auto accept(rio::context &ctx, rio::Tcp_socket &listener)
{
    ...
    auto *req = new Accept_req{ .header = { .call = &Accept_req::on_complete },
        .state = s,
        .client_addr = {},
...
{
    return rio::fut::make(Accept_all_impl<std::decay_t<Handler>>{ &ctx, &listener, std::forward<Handler>(handler) },
        [](auto &s) { return s.poll(); });
}
...
auto stop_accept_after(rio::context &ctx, rio::Tcp_socket &listener, std::chrono::duration<Rep, Period> timeout)
{
    ...
    auto *accept_req = new Accept_req{ .header = { .call =
                                                       [](rio::internals::uring_request_header *ptr, int res) {
                                                           auto *self = reinterpret_cast<Accept_req *>(ptr);
                                                           if (res == -ECANCELED) {
                                                               rio::Promise<Async_state<Val_type>> p{
                                                                   .state = self->state
                                                               };
    ...
    io_uring_prep_accept(sqe_accept,
        listener.fd.native_handle(),
