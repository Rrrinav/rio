...
export enum class s_opt : uint32_t {
    ...
    // This is usually what people mean by "server" on modern Linux
    ...
    client          = client_v4 // Default client to v4 for compatibility
};

// Boilerplate for bitwise ops
...
export auto accept(Tcp_socket &s, s_opt options = s_opt::none) -> result<Tcp_socket>
{
...
// Batch accept: accept multiple connections in one call (Linux 4.5+)
#ifdef __linux__
export auto accept_many(Tcp_socket &s, std::span<Tcp_socket> out, s_opt options = s_opt::none) -> result<size_t>
{
    const int flags = SOCK_CLOEXEC | (has(options, s_opt::nonblock) ? SOCK_NONBLOCK : 0);
...
export auto accept_all(Tcp_socket &s, auto &&handler, s_opt options = s_opt::none) -> result<size_t>
{
    const int flags = SOCK_CLOEXEC | (has(options, s_opt::nonblock) ? SOCK_NONBLOCK : 0);
